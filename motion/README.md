...
Five standalone personality performances are available in every target and in
the site/Storybook galleries: `slow-blink` (3.2 s, trusting eye greeting),
`sleepy` (4.4 s, doze and pretend to be awake), `proud` (3.6 s, quiet satisfaction),
`feign-ignore` (3.8 s, look away while listening), and `ring-side-eye` (4.8 s,
the ring moves again after the cat looks up). Each returns to the neutral pose.
They use the existing face geometry (including the happy eye contours for proud),
rigid star muzzle and synchronized ring halves. The polished acting includes a
held affectionate lean after the slow blink, a failed attempt to stay awake,
a proud eye-smile, an asymmetric listening ear and a narrowed second look at the
ring. These complete performances are standalone clips, not emotion overlays
or new inputs in the combined machine. Preview them in `polish-review.html`
generated by `node scripts/motion-review.mjs`, or in the target players.

Long eye performances (`sleepy`, `slow-blink`, `feign-ignore`, `ring-side-eye`)
use the `drowsy` and `resting-eyes` aperture contours instead of scaling the
entire eye into an oval. The upper contour descends while the lower edge stays
anchored. Compatible cubic topology keeps these poses morphable in all targets;
pupils retain their original dimensions and are clipped by the changing aperture.
`node scripts/motion-contact-review.mjs --eyes` creates enlarged eye sheets for
all clips. The `--eyes` samples include partial closure, held closure and reopening.
