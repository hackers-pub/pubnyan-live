...
<style>iframe{height:min(52vh,480px);aspect-ratio:auto}</style>
<div class="controls"><label for="clip">동작</label><select id="clip"></select><button id="play" type="button">재생</button><button id="reset" type="button">처음으로</button><label for="rate">속도</label><select id="rate"><option value="1">1×</option><option value="0.5">0.5×</option><option value="0.25">0.25×</option></select><label style="display:flex;align-items:center;gap:8px"><input id="eyes" type="checkbox" style="min-width:20px;flex:none">눈 확대</label></div>
...
const labels={'slow-blink':'느린 눈인사',sleepy:'졸다가 아닌 척',proud:'은근한 뿌듯함','feign-ignore':'못 들은 척','ring-side-eye':'링 눈치 보기'};
for(const entry of entries){const option=document.createElement('option');option.value=entry.name;option.textContent=labels[entry.name]?labels[entry.name]+' · '+entry.name:entry.name;$('clip').append(option);}
const requested=new URLSearchParams(location.search).get('clip');
$('clip').value=entries.some(e=>e.name===requested)?requested:'slow-blink';
function entry(){return entries.find(e=>e.name===$('clip').value)}
function paint(){const poseTime=Math.min(t,entry().duration);for(const frame of iframes){const zoom=$('eyes').checked;frame.style.height=zoom?'auto':'';frame.style.aspectRatio=zoom?'210 / 100':'';const svg=frame.contentDocument?.querySelector('svg');if(svg){svg.dataset.fullViewBox??=svg.getAttribute('viewBox');svg.setAttribute('viewBox',zoom?'105 70 210 100':svg.dataset.fullViewBox);}for(const a of frame.contentDocument?.getAnimations()??[]){a.pause();a.currentTime=poseTime*1000;}}$('time').value=poseTime;$('clock').textContent=poseTime.toFixed(2)+' s';}
function pause(){playing=false;$('play').textContent='재생'}
function load(){pause();t=0;const e=entry();$('time').max=e.duration;$('old').hidden=!e.before;document.querySelector('.frames').style.gridTemplateColumns=e.before?'':'1fr';for(const [i,frame]of iframes.entries()){frame.onload=paint;frame.srcdoc='<style>html,body{margin:0;overflow:hidden}svg{display:block;width:100%;height:100vh}*,svg *{animation-play-state:paused!important}</style>'+(i===0?(e.before??''):e.after);}paint();}
